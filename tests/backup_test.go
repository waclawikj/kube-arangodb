...
func waitUntilBackup(ci versioned.Interface, name, ns string, predicate func(*backupApi.ArangoBackup, error) error, timeout ...time.Duration) (*backupApi.ArangoBackup, error) {
	...
	op := func() error {
		obj, err := ci.BackupV1alpha().ArangoBackups(ns).Get(name, meta.GetOptions{})
		result = obj
		if predicate != nil {
...
func timeoutWaitForBackups(t *testing.T, backupClient backupClient.ArangoBackupInterface, labels meta.LabelSelector, size int) func() error {
	return func() error {
		backups, err := backupClient.List(meta.ListOptions{LabelSelector: meta.FormatLabelSelector(&labels)})
		if err != nil {
			return err
		}
...
func BackupCreateBackup(t *testing.T, deployments ... *api.ArangoDeployment) {
	ctx := arangod.WithRequireAuthentication(context.Background())
	deployment := deployments[0]

	backup := helper.NewArangoBackup(t, "create-test", deployment)

	defer helper.CleanArangoBackup(t, backup.GetName())

	backup = helper.CreateArangoBackup(t, backup)

	helper.WaitUntilArangoBackupIsReady(t, backup.GetName())

	backup = helper.RefreshArangoBackup(t, backup)

	client := mustNewArangodDatabaseClient(ctx, helper.KubernetesClient(t), deployment, t, nil)
	meta := helper.GetMetaByBackup(t, client, ctx, backup)

	helper.CompareBackup(t, meta, backup)
}

func BackupCreateMultipleBackups(t *testing.T, deployments ... *api.ArangoDeployment) {
	...
	backups := make([]string, size * len(deployments))
	...
	for id := 0; id < size; id++ {
		for deploymentID, deployment := range deployments {
			...
			backup = helper.CreateArangoBackup(t, backup)
			backups[deploymentID * size + id] = backup.GetName()
		}
	}

	helper.WaitUntilArangoBackupsAreReady(t, backups...)


	for id := 0; id < size; id++ {
		for deploymentID, deployment := range deployments {
			backup := helper.GetBackupByName(t, backups[deploymentID * size + id])
...
func Test_New_Backup(t *testing.T) {
	helper.MarkSmokeTest(t, false)
	...
	deploymentLabels1 := map[string]string{
		"COMMON": "1",
		"TEST":   "1",
	}

	deploymentWrapper1 := helper.NewArangoDeploymentWrapper("test-backup", func(deployment *api.ArangoDeployment) {
		deployment.Spec.Mode = api.NewMode(api.DeploymentModeCluster)
		deployment.Spec.DBServers.Count = util.NewInt(2)
		deployment.Spec.DBServers.Args = cmd
		deployment.Spec.Coordinators.Count = util.NewInt(2)
		deployment.Spec.Coordinators.Args = cmd
		deployment.Spec.Agents.Args = cmd
		deployment.Labels = deploymentLabels1
	})

	deploymentLabels2 := map[string]string{
		"COMMON": "1",
		"TEST":   "2",
	}

	deploymentWrapper2 := helper.NewArangoDeploymentWrapper("test-backup-two", func(deployment *api.ArangoDeployment) {
		deployment.Spec.Mode = api.NewMode(api.DeploymentModeCluster)
		deployment.Spec.DBServers.Count = util.NewInt(2)
		deployment.Spec.DBServers.Args = cmd
		deployment.Spec.Coordinators.Count = util.NewInt(2)
		deployment.Spec.Coordinators.Args = cmd
		deployment.Spec.Agents.Args = cmd
		deployment.Labels = deploymentLabels2
	})
	...
	deploymentWrapper1.Run(t, func(t *testing.T, deployment1 *api.ArangoDeployment) {
		deploymentWrapper2.Run(t, func(t *testing.T, deployment2 *api.ArangoDeployment) {
			...
			helper.SkipIfNotBackup(t, client1, ctx)
			helper.SkipIfNotBackup(t, client2, ctx)

			// Run other cases

			t.Run("Create Multiple Backups", func(t *testing.T) {
				BackupCreateMultipleBackups(t, deployment1, deployment2)
			})

			t.Run("Create Backup", func(t *testing.T) {
				BackupCreateBackup(t, deployment1, deployment2)
			})
		})
	})
	//

...
func TestPVCChangeStorage(t *testing.T) {
	...
	// Create deployment
	_, err = arangoClient.DatabaseV1alpha().ArangoDeployments(k8sNameSpace).Create(depl)
	...
	require.NoError(t, err, fmt.Sprintf("Deployment not running in time: %s", err)) // <-- fails here at the moment
	require.NotNil(t, depl.Spec.DBServers.VolumeClaimTemplate)

	// Update deployment
	depl.Spec.DBServers.VolumeClaimTemplate.Spec.StorageClassName = util.NewString(newStorageClassName)
	_, err = arangoClient.DatabaseV1alpha().ArangoDeployments(k8sNameSpace).Update(depl)
...
func TestPVCChangeStorageDeprecated(t *testing.T) {
	...
	// Update deployment
	depl.Spec.DBServers.StorageClassName = util.NewString(newStorageClassName)
	_, err = arangoClient.DatabaseV1alpha().ArangoDeployments(k8sNameSpace).Update(depl)
	require.NoError(t, err, "failed to update deployment: %s", err)
